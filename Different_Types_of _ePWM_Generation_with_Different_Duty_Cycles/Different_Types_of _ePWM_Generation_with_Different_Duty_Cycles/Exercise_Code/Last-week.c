...
Generate an integrator signal for square wave at EPWM3. Use EPWM5 to find the inductor current values of avg,peak and valley.
Use EPWM4 as master to EPWM5.
...

{
    ...
    EPwm3Regs.TBCTL.bit.HSPCLKDIV=4;

    EPwm3Regs.TBCTL.bit.CTRMODE = 2;   // up-down count

    EPwm3Regs.AQCTLA.all=0x0060;     //0000 0000 0110 0000

    EPwm3Regs.TBPRD=19532/2;           // Epwm3 time period = 25 ms
...
//    EPwm3Regs.ETSEL.bit.INTEN = 1;     // **************************************normally done this way
//
...
//    EPwm4Regs (MASTER) configuration:

    EPwm4Regs.TBCTL.bit.CLKDIV = 7;

    EPwm4Regs.TBCTL.bit.HSPCLKDIV = 6;

    EPwm4Regs.TBCTL.bit.CTRMODE = 2;    //up-down count mode

    EPwm4Regs.TBPRD = 65104/2;          // Time period = 1sec

    EPwm4Regs.CMPA.bit.CMPA = 0.25*65104;

    EPwm4Regs.AQCTLA.all = 0x0060;       // out=0 when lesser, out=1 when greater

    EPwm4Regs.TBCTL.bit.PHSEN = 0;

    EPwm4Regs.TBCTL.bit.SYNCOSEL = 1;     // used to synchronize


//    EPwm5Regs (SLAVE) configuration:
